  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        ...
        // Show linking prompt
        setLinkingProvider(err.existingProvider);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 relative my-8">
        ...
        {linkingProvider && pendingCredential && (
          <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-start gap-2 mb-3">
              <Link className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
              <div>
                <p className="font-medium text-blue-900">Link Your Accounts</p>
                <p className="text-sm text-blue-700 mt-1">
                  An account with <strong>{pendingEmail}</strong> already exists.
                  Sign in with your existing method below to link both accounts together.
                </p>
              </div>
            </div>
            <div className="space-y-2">
              {linkingProvider === 'google.com' && (
                <button
                  onClick={() => handleSocialLogin('google')}
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition text-sm font-medium"
                >
              ...
              {linkingProvider === 'facebook.com' && (
                <button
                  onClick={() => handleSocialLogin('facebook')}
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-[#1877f2] text-white rounded-lg hover:bg-[#166fe5] transition text-sm font-medium"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                  Sign in with Facebook to link accounts
                </button>
              )}
              {linkingProvider === 'password' && (
                <p className="text-sm text-blue-700">
                  Enter your email and password below to sign in and link accounts.
                </p>
              )}
            </div>
            <button
